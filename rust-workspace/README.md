...
General-purpose Rust development workspace.

## Image

[`ghcr.io/clankerguru/devcontainer:rust-latest`](https://github.com/ClankerGuru/devcontainer/pkgs/container/devcontainer)

Built from [`ClankerGuru/devcontainer`](https://github.com/ClankerGuru/devcontainer) — `rust/Dockerfile`.
...
| IDE | Access | Default |
|-----|--------|---------|
| JetBrains Gateway | RustRover | on |
...
Templates live in [`ClankerGuru/terraform`](https://github.com/ClankerGuru/terraform) under `rust-workspace/`.

To update:

```bash
# Edit the template
vim rust-workspace/main.tf

# Push to Coder
coder templates push rust-workspace --directory ./rust-workspace
...
```bash
coder templates edit rust-workspace \
  --display-name "Rust" \
...
From the dashboard: Templates > Rust > Create Workspace.

From the CLI:

```bash
# With all defaults
coder create dev --template rust-workspace --use-parameter-defaults

# Interactive (choose agents and IDEs)
coder create dev --template rust-workspace
```
